...
const ViewPredictions = () => {
  ...
  const fetchPredictions = async () => {
    try {
      ...
      const res = await axios.get("https://farmer-final-bkp2.onrender.com/api/cropprediction", {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`https://farmer-final-bkp2.onrender.com/api/cropprediction/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `https://farmer-final-bkp2.onrender.com/api/cropprediction/${editingPrediction._id}`,
        editingPrediction,
        { headers: { Authorization: `Bearer ${token}` } }
      );
